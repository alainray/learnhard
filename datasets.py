...
def get_dataloaders(args):
    ...
    test_data = data[args.dataset](transform=preproc['test'][args.res], 
                            root=root, 
                            train=False if "cifar" not in args.dataset else True, 
                            download=True)
...
def CIFARIdx(cl, args):
    ...
    class DatasetCIFARIdx(cl):
        ...
        def __getitem__(self, index: int) -> Tuple[Any, Any]:
            ...
            label = self.scores[index] if args.label_type=="score" else (digitize(self.scores[index],bins) - 1).astype(np.longlong)
